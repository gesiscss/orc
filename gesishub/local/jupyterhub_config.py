...
#c.JupyterHub.data_files_path = '/home/kenan/anaconda3/envs/ghub/share/jupyterhub'
...
# dummy projects for any logged in user
from jupyterhub.spawner import LocalProcessSpawner as BaseLocalProcessSpawner
class LocalProcessSpawner(BaseLocalProcessSpawner):
    def get_state_field(self, name):
        if name == "projects":
            # add example projects for local dev
...
extra_handlers = [
    (r'{}(.*)'.format(template_vars["static_nginx"]), web.StaticFileHandler, {'path': repo_dir + '/load_balancer/static'}),
]
